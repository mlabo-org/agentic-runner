...
- `<git-root>/.agentic-runner/task.md`: current task SSOT, including purpose, scope, intake-only `workflow_profile` / `workflow_domain` / `workflow_domain_contract`, route control-plane fields (`route_class`, `agentic_runner_layer`, `controlled_workflows`, `execution_owner`, `handoff_artifacts`, `resume_checkpoint`, `cross_workflow_completion`), semantic `work_type`, non-goals, completion conditions, permitted hierarchy mode, and supervision defaults when the task uses long-running or delegated assignments.
...
5. Run intake with explicit isolation keys, optional intake-only workflow profile metadata, optional route-state metadata, and optional semantic work metadata:
   `node /Users/suzukimakoto/plugins/agentic-runner/bin/agentic-runner.mjs intake --target-cwd <jobsite> --workflow-profile <default|coding-agent|plugin-source> --route-class <unknown|coding|article|video|plugin-source|mixed> --primary-workflow <agentic-runner|coding-agents|agentic-structciv|codex-video> --work-type <auto|documentation|source-change|debug> --task <task> --task-id <id> --epoch <epoch> --scope <scope>`.
...
9. For `assign`, `collect`, `run`, or `orchestrate`, record operational packets in the jobsite repository's `.agentic-runner/runner.md`. Do not pass `--workflow-profile`, `--route-class`, or `--primary-workflow` to runner commands; packets inherit the current intake's workflow profile and route state and must stop if state contains an unknown workflow profile or route owner. Use `--specialist-owner <agentic-runner|coding-agents|agentic-structciv|codex-video>` to name the subordinate execution owner when the route is mixed. Use `--feature-profile <id>` only as an optional scoped overlay for that assignment instance, and keep missing profiles as `feature_profile: none`. Use `--work-type <id>` only as semantic command metadata for gate classification, and keep missing work types as `work_type: auto`.
   ...
   - Preserve legacy runner compatibility only for simple path-only values such as `README.md`, `allowed/`, `bin/agentic-runner.mjs tests/workflow-state.test.mjs`, plus whole-repo aliases `.`, `repo`, and `whole repo`.
