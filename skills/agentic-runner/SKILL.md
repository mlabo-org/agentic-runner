...
- Resolve the jobsite's Git root before writing workflow state. The active state root is the jobsite repository's `<git-root>/.agentic-runner/`, even when `invocation_cwd` is a plugin, tooling, or parent repository.
...
- Before creating or updating the jobsite repository's `<git-root>/.agentic-runner/`, ensure that repository's `.git/info/exclude` ignores `.agentic-runner/`. Add only that local exclude entry when it is missing.
...
- The first action after trigger is target resolution followed by project intake. Determine `invocation_cwd`, resolve the jobsite from the explicit target or default cwd rule, read the local `AGENTS.md` chain that applies to the jobsite when available, inspect the jobsite repository shape, check Git state, resolve `<git-root>`, inspect existing `.agentic-runner` state, inspect `.git/info/exclude`, and identify legacy `docs/codex` material only as migration input.
...
- Maintain `<git-root>/.agentic-runner/` as the workflow SSOT for the active job.
...
- Every `state_retired` workflow transition, and every separately performed cancellation, interruption, or replacement action, must record exactly one allowed `cancel_reason`: `completed_retire`, `user_stop`, `safety_stop`, `scope_violation`, `stale_timeout`, `blocker_or_failure`, or `stale_premise`. `continuation_expected` must record `cancel_reason: none`.
...
- Parent ownership remains intact throughout supervision. The parent owns policy, cancellation judgment, workflow-state lifecycle disposition, replacement assignment, result acceptance, and final integration; workers and descendants provide telemetry and integration material but do not set their final lifecycle disposition.
...
5. Run intake with explicit isolation keys, optional route-state metadata, and optional semantic work metadata:
   `node /Users/suzukimakoto/plugins/agentic-runner/bin/agentic-runner.mjs intake --target-cwd <jobsite> --route-class <unknown|coding|article|video|plugin-source|mixed> --primary-workflow <agentic-runner|coding-agents|agentic-structciv|codex-video|safe-custom-id> [--controlled-workflows <comma-separated-safe-ids>] --work-type <auto|documentation|source-change|debug> --task <task> --task-id <id> --epoch <epoch> --scope <scope>`.
6. Run doctor:
...
7. Print handoff when needed:
   `node /Users/suzukimakoto/plugins/agentic-runner/bin/agentic-runner.mjs handoff --target-cwd <jobsite> --task-id <id>`.
...
9. For `assign`, `collect`, `run`, or `orchestrate`, record operational packets in the jobsite repository's `.agentic-runner/runner.md`. Do not pass `--route-class` or `--primary-workflow` to runner commands; packets inherit the current route state and must stop if state contains an invalid route owner id. Use `--specialist-owner <safe-owner-id>` to name the subordinate execution owner when the route is mixed. Use `--work-type <id>` only as semantic command metadata for gate classification, and keep missing work types as `work_type: auto`. On `collect`, use `--lifecycle-disposition state_retired --cancel-reason <allowed-reason>` for a terminal workflow-state transition or `--lifecycle-disposition continuation_expected` when more work may follow; omission defaults safely to `continuation_expected`.
...
3. Resolve the jobsite repository's `<git-root>` and run project intake before editing: repository status, applicable instructions, current `.agentic-runner` state, `.git/info/exclude` status, legacy `docs/codex` migration input, source/cache boundaries, and risk level.
...
7. Execute or coordinate work according to `.agentic-runner/todo.md`.
