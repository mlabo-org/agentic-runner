...
- Do not trigger this skill merely because a task belongs to coding, article writing, or video production. Trigger it when Agentic Runner is needed to decide, route, supervise, resume, hand off, or audit subordinate workflows such as `coding-agents`, Agentic StructCiv, or CodexVideo.
- Do not treat Agentic Runner as a peer of `coding-agents`, Agentic StructCiv, or CodexVideo. Agentic Runner is the control-plane above those execution workflows.
...
- Agentic Runner is the upper control-plane. It owns route classification, controlled workflow selection, handoff artifacts, supervision state, resume checkpoints, stale-route handling, and final cross-workflow audit.
- `coding-agents`, Agentic StructCiv, and CodexVideo are subordinate execution workflows controlled by Agentic Runner route state. They own production artifacts and verification evidence inside their domains; they do not own Agentic Runner's final policy, route map, or cross-workflow completion judgment.
- `coding-agents` is the subordinate execution workflow for code generation, debugging, source repair, tool implementation, tests, and source/cache/runtime-boundary work.
- Agentic StructCiv is the subordinate execution workflow for article, BLOG, WordPress, SWELL, SEO-oriented writing, structure, drafting, and editorial assembly.
- CodexVideo is the subordinate execution workflow for video, short-form production, source-to-video, voice, narration, subtitles, TTS, Remotion-like production, and audiovisual handoff.
- A single-domain request may route to one subordinate workflow. Mixed requests must keep Agentic Runner as the control-plane and issue explicit subordinate execution-owner packets for each domain rather than collapsing the work into Agentic Runner itself.
...
- Known controlled workflow ids are `agentic-runner`, `coding-agents`, `agentic-structciv`, and `codex-video`. Unknown `execution_owner` or `specialist_owner` values in current route state are validation failures.
...
- `--specialist-owner` is a packet-level subordinate execution-owner selector for `assign`, `collect`, `run`, and `orchestrate`. Mixed routes require it before appending runner state.
...
- Intake may include `--route-class <id>`, `--primary-workflow <id>`, and `--work-type <id>`. `assign`, `collect`, `run`, and `orchestrate` may include `--specialist-owner <id>` and `--work-type <id>`. No other routing overlay is part of the contract.
...
5. Run intake with explicit isolation keys, optional route-state metadata, and optional semantic work metadata:
   `node /Users/suzukimakoto/plugins/agentic-runner/bin/agentic-runner.mjs intake --target-cwd <jobsite> --route-class <unknown|coding|article|video|plugin-source|mixed> --primary-workflow <agentic-runner|coding-agents|agentic-structciv|codex-video> --work-type <auto|documentation|source-change|debug> --task <task> --task-id <id> --epoch <epoch> --scope <scope>`.
6. Run doctor:
...
9. For `assign`, `collect`, `run`, or `orchestrate`, record operational packets in the jobsite repository's `.agentic-runner/runner.md`. Do not pass `--route-class` or `--primary-workflow` to runner commands; packets inherit the current route state and must stop if state contains an unknown route owner. Use `--specialist-owner <agentic-runner|coding-agents|agentic-structciv|codex-video>` to name the subordinate execution owner when the route is mixed. Use `--work-type <id>` only as semantic command metadata for gate classification, and keep missing work types as `work_type: auto`.
