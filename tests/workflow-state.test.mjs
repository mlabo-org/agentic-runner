...
test("source identity rejects legacy Coding Agents identity misuse but allows controlled routed workflow names", () => {
  ...
  assert.deepEqual(
    findLegacyIdentityMisuse(".agents/plugins/marketplace.json", `"name": "${["coding", "agents"].join("-")}"`),
    [],
  );
  assert.ok(
    findLegacyIdentityMisuse(
      ".agents/plugins/marketplace.json",
      `"displayName": "${["Coding", "Agents"].join(" ")}"`,
    ).length,
  );
...
test("doctor reports explicit self-host gate for the Agentic Runner source repo", () => {
  ...
  const copiedBinDir = path.join(repo, "bin");
  const copiedCli = path.join(copiedBinDir, "agentic-runner.mjs");
  try {
    mkdirSync(copiedBinDir, { recursive: true });
    ...
    const intake = spawnSync(
      ...
      [
        ...
        repo,
        "--task",
        "verify hermetic self-host doctor state",
        "--task-id",
        "self-host-doctor",
        "--epoch",
        "e1",
        "--scope",
        "scope:v1 paths=README.md",
        "--work-type",
        "source-change",
      ],
      { cwd: repo, env: process.env, encoding: "utf8", maxBuffer: 1024 * 1024 },
    );
    assert.equal(intake.status, 0, intake.stderr);
    assert.match(intake.stdout, /ok self_host_target: true/);
    assert.match(intake.stdout, /ok self_host_gate: explicit_self_host_target/);

    const result = spawnSync(process.execPath, [copiedCli, "doctor", "--target-cwd", repo], {
      cwd: repo,
      env: process.env,
      encoding: "utf8",
      maxBuffer: 1024 * 1024,
...
function findLegacyIdentityMisuse(file, text) {
  ...
  const allowControlledMarketplaceName = file === ".agents/plugins/marketplace.json";
  const rules = [
    [
      new RegExp(`"name"\\s*:\\s*"${escapeRegExp(routedName)}"`, "i"),
      "plugin name uses legacy routed workflow id",
      !allowControlledMarketplaceName,
    ],
  ...
  return rules
    .filter(([pattern, , applies = true]) => applies && pattern.test(text))
